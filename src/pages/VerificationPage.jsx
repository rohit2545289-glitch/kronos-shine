...
function VerificationPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      // ✅ CORRECT PATH: mobile_verification/{deviceId}
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Loading verification...</p>
        <style>{`
          @keyframes spin {
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        ...
        <p style={{ fontSize: '13px' }}>Failed to load verification data</p>
  ...
  if (!data) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        <span style={{ fontSize: '32px', display: 'block', marginBottom: '4px' }}>📭</span>
        <p style={{ fontSize: '13px' }}>No verification data found</p>
      </div>
    );
  }
    ...
    <div style={{ padding: '4px 0' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>👩 Mother's Name</span>
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>
            {data.motherName || 'N/A'}
          </span>
        </div>
